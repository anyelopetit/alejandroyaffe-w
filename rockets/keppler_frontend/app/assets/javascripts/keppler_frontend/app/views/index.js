...
function parallaxOnMouseMove(elem) {
  elem.closest('section').mousemove(function (event) {
    elem.closest('section').css({
      'background-position': `${(($(window).width() / 2) - event.pageX) * .001}px, ${(($(window).width() / 2) - event.pageY) * .001}px`
    })
    elem.css({
...
function carousel3d(carousel, infinite = false) {
  ...
  if (infinite) {
    ...
    setTimeout(carousel3d(carousel, infinite), 5000)
  }
}
